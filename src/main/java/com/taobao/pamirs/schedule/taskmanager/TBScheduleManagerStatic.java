...
public class TBScheduleManagerStatic extends TBScheduleManager {
    ...
    /**
     * 首先启动一个独立的线程，判断isRuntimeInfoInitial标志位判断是否已经初始化数据，如果没有则leader调度器执行initialRunningInfo()，
     * 删除/TaskItem目录，根据ScheduleTaskType，获取到的任务项数组，创建任务项节点，同时在/taskItem下设置leader数据。
     *
     * @throws Exception
     */
